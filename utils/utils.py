...
def train_val_test_split_4(mat_dir, kfold = 5, fold = 0, rep=100):
    
    subjects = loadmat(mat_dir)
    con_id = list(subjects['idx_1'])
    emci_id = list(subjects['idx_2'])
    lmci_id = list(subjects['idx_3'])
    pat_id = list(subjects['idx_4'])
    
    sub_list = con_id + emci_id + lmci_id + pat_id
    label = list(np.ones(len(emci_id)))+list(np.zeros(len(con_id))) + list(2*np.ones(len(lmci_id))) + list(3*np.ones(len(pat_id)))
    list1, list2 = (list(t) for t in zip(*sorted(zip(sub_list, label))))
